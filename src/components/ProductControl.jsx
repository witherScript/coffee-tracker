...
class ProductControl extends React.Component {
  ...
  handleSellingingProduct = (id) => {
    const toSell = this.state.mainProductList.filter(prod => prod.id === id)[0]; //{ quant: 6}
    let updatedQuant;
  ...
  render(){
    ...
    if (this.state.formVisible) {
      currentlyVisibleState = <NewBurlapForm onProductCreation={this.handleProductCreation} />;
      buttonText = "Return to List";
    } else if (this.state.selectedProduct != null) {
      currentlyVisibleState = <BurlapDetail product={this.state.selectedProduct} onClickingEdit={this.handleEditClick}  />
    }
      ...
      currentlyVisibleState = (
        <>
          <ProductList productList={this.state.mainProductList} onPurchase={this.handleSellingProduct} onProductSelection={this.handleChangingSelectedBurlap} />
        </>
      );
